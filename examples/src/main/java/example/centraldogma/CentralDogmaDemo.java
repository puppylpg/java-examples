...
//        getFile();
    ...
    private static void getFile() {
        CompletableFuture<Entry<String>> future = dogma.getFile("bid-server", "src/main/resources/config", Revision.HEAD, Query.ofText("/e-greedy/flow-ratio"));
...
//        );
        ...
        clientWrapper.watchConfigWithoutInitial(
                "bid-server", "src/main/resources/config", "/unknown",
                ((revision, s) -> System.out.print(s))
        );

        clientWrapper.watchConfigWaitingInitialLimited(
                "bid-server", "src/main/resources/config", "/e_greedy/flow_ratio",
                ((revision, s) -> System.out.print(s))
        );
